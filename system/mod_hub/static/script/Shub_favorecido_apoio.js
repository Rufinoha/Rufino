...
let id_favorecido = null;
window.addEventListener("message", async (event) => {
  if (event.data?.grupo === "dadosApoio") {
    id_favorecido = event.data.id || null;
    if (id_favorecido) await carregarFavorecido(id_favorecido);
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  await GlobalUtils.carregarCategorias("Favorecido");
...
async function excluirFavorecido() {
  ...
  if (confirm.isConfirmed) {
    try {
      ...
      const json = await resp.json();
     if (json.sucesso) {
        Swal.fire("Excluído", "Registro removido.", "success").then(() => {
          window.parent.postMessage({ grupo: "atualizarTabela" }, "*");
          GlobalUtils.fecharJanelaApoio(1);
        });
      } else {
