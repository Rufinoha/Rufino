...
if (typeof window.LivroDiarioHub === "undefined") {
  window.LivroDiarioHub = {
    ...
    configurarEventos: function () {
      ...
      document.querySelector("#ob_btnIncluir").addEventListener("click", () => {
        ...
        if (janelaLivroIncluir && !janelaLivroIncluir.closed) {
          janelaLivroIncluir.focus();
          return;
        }

        janelaLivroIncluir = window.open(
          '/livro_diario/incluir',
          'NovaContaLivroDiario',
  ...
  document.querySelector("#ob_listaLivroDiario").addEventListener("click", async function (e) {
    if (e.target.classList.contains("btnEditar")) {
      ...
      if (window.janelaEditarLivro && !window.janelaEditarLivro.closed) {
        window.janelaEditarLivro.focus();
      } else {
        window.janelaEditarLivro = window.open(
          "/livro_diario/editar",
          "EditarLivroDiario",
          `width=${largura},height=${altura},top=${topo},left=${esquerda},resizable=no`
        );
